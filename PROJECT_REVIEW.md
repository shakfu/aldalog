...
1. **Lua Sandbox**
   The embedded Lua has full standard library access (`io`, `os`). Malicious `.aldev/init.lua` could execute arbitrary commands.
...
Aldev is a well-engineered project that successfully combines a lightweight text editor with music live-coding capabilities. The codebase demonstrates professional quality with:
