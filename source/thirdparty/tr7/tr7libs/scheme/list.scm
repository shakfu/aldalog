...
;;;     -Olin
;;; Version modified for r7rs by José Bollo, 2021
      ...
      (define (length+ x)
        (let ((len (length* x)))
          (cond
            ((>= len 0) len)
            ((= len -1) #f)
            (else (error "dotted list" length+ x)))))
  )
  (else
...
(define (check-arg pred val caller)
  (let lp ((val val))
    (if (pred val) val (lp (error "Bad argument" val pred caller)))))
(define (:optional val def) (if (pair? val) (car val) def))
...
(define (iota count . maybe-start+step)
  ...
  (let ((start (if (null? maybe-start+step) 0 (car maybe-start+step)))
        (step (if (or (null? maybe-start+step) (null? (cdr maybe-start+step))) 1 (cadr maybe-start+step))))
