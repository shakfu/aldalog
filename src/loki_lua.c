...
/* Lua API: loki.alda.cleanup() - Cleanup alda subsystem */
...
/* Lua API: loki.alda.is_initialized() - Check if alda is initialized */
...
/* Lua API: loki.alda.eval(code, callback) - Evaluate alda code asynchronously */
...
/* Lua API: loki.alda.eval_sync(code) - Evaluate alda code synchronously (blocking) */
...
/* Lua API: loki.alda.stop(slot) - Stop playback (slot=-1 or nil for all) */
...
/* Lua API: loki.alda.stop_all() - Stop all playback */
...
/* Lua API: loki.alda.is_playing() - Check if any slot is playing */
...
/* Lua API: loki.alda.active_count() - Get number of active playback slots */
...
/* Lua API: loki.alda.set_tempo(bpm) - Set global tempo */
...
/* Lua API: loki.alda.get_tempo() - Get current tempo */
...
/* Lua API: loki.alda.set_synth(enable) - Enable/disable built-in synth */
...
/* Lua API: loki.alda.load_soundfont(path) - Load a SoundFont file */
...
/* Lua API: loki.alda.get_error() - Get last error message */
...
static void lua_register_alda_module(lua_State *L) {
    ...
    lua_newtable(L);  /* Create alda subtable */
...
/* Load init.lua: try override, .aldev/init.lua (local), then ~/.aldev/init.lua */
int loki_lua_load_config(lua_State *L, const struct loki_lua_opts *opts) {
    ...
    /* Try local .aldev/init.lua first (project-specific) */
    if (project_root) {
        snprintf(init_path, sizeof(init_path), "%s/.aldev/init.lua", project_root);
    } else {
        snprintf(init_path, sizeof(init_path), ".aldev/init.lua");
    }
    ...
    if (!loaded) {
        ...
        if (home && home[0] != '\0') {
            snprintf(init_path, sizeof(init_path), "%s/.aldev/init.lua", home);
            if (access(init_path, R_OK) == 0) {
                if (luaL_dofile(L, init_path) != LUA_OK) {
                    const char *err = lua_tostring(L, -1);
                    loki_lua_report(opts, "Lua init error (%s): %s", init_path, err ? err : "unknown");
...
static void loki_lua_extend_path(lua_State *L, const struct loki_lua_opts *opts) {
    ...
    const char *project_root = (opts && opts->project_root && opts->project_root[0] != '\0')
                                   ? opts->project_root
                                   : ".";
    int wrote = snprintf(addition + used, remaining, "%s/.aldev/?.lua;%s/.aldev/?/init.lua;",
    ...
    if (home && home[0] != '\0' && remaining > 1) {
        wrote = snprintf(addition + used, remaining, "%s/.aldev/?.lua;%s/.aldev/?/init.lua;",
                         home, home);
        if (wrote > 0 && (size_t)wrote < remaining) {
            used += (size_t)wrote;
            remaining -= (size_t)wrote;
